...
import { appendActionToGoogleSheet, extractCityFromUnit, getCachedGoogleAccessToken } from '../lib/googleSheets';
...
const NewApproach: React.FC<NewApproachProps> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Registrar em Ações Diárias do Turno
      const actionNewId = `action-approach-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
      const currentUnit = user?.unidade || 'FORÇA TÁTICA';
      const currentCity = extractCityFromUnit(currentUnit);
      
      const actionPayload = {
        id: actionNewId,
        categoria: 'ABORDAGENS',
        tipo_acao: 'PESSOAS ABORDADAS',
        unidade: currentUnit,
        cidade: currentCity,
        vtr_service_id: '',
        equipe_detalhes: user?.nome || 'EQUIPE',
        comandante: user?.nome || 'CMT',
        viatura_prefixo: '-',
        criado_por_id: user?.id || 'unknown',
        criado_por_nome: user?.nome || 'unnamed',
        quantidade: 1,
        detalhes: JSON.stringify({ nome: individualData.nome.toUpperCase(), documento: individualData.documento || '-', local: approachData.local || '-' }),
        created_at: new Date().toISOString(),
        data_selecionada: approachData.data || new Date().toISOString().split('T')[0]
      };

      const dailyActionRef = doc(db, 'daily_actions', actionNewId);
      batch.set(dailyActionRef, actionPayload);

      await batch.commit();

      // Trigger instant Google Sheets append in background
      try {
        const token = getCachedGoogleAccessToken();
        appendActionToGoogleSheet(actionPayload, token || undefined).catch(e => console.warn('Google Sheets background sync notice:', e));
      } catch (e) {}
