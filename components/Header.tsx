...
import { BookOpen, Eye, Network, ClipboardList, ShieldAlert } from 'lucide-react';
  ...
  const NavIcons = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className={`flex items-center ${isMobile ? 'justify-around w-full' : 'space-x-1 sm:space-x-2'}`}>
      ...
      {user && user.role !== UserRole.OPERATOR && (
        <Link 
          to="/p3-monitoramento" 
          className={`p-3 sm:p-2 rounded-xl transition-all flex items-center justify-center ${location.pathname === '/p3-monitoramento' ? 'bg-red-600 text-white sm:bg-red-600/10 sm:text-red-600' : 'text-red-500 hover:bg-red-50 hover:text-red-700'}`}
          title="Monitoramento e Auditoria P3"
        >
          <ShieldAlert size={isMobile ? 22 : 18} />
        </Link>
      )}
